...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SwiGLUFFN,AttentionModulationHead
from .conv import Conv, DWConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init

__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment", "EnhancedDetect"

from ...utils.loss import AttentionModulationLoss
...
class EnhancedDetect(Detect):
    """
    增强的检测头，集成attention调制功能
    继承自标准的Detect类，添加训练时的attention调制能力
    """
    ...
    def __init__(self, nc=80, ch=()):
        ...
        # 导入AttentionModulationHead

        # 为每个检测层创建attention调制头
        self.attention_heads = nn.ModuleList(
            AttentionModulationHead(x) for x in ch
        )

        # 添加attention调制损失
        self.attention_loss_fn = AttentionModulationLoss()

    def forward(self, x, coords=None, targets=None):
        """
        前向传播
        Args:
            x: 多尺度特征图列表
            coords: 训练时的目标框坐标 (B, N, 4)
            targets: 训练时的目标标签信息
        Returns:
            训练时: (detection_outputs, original_features, modulated_features)
            推理时: 标准检测输出
        """
        if self.training and coords is not None:
            ...
            # 对每个尺度的特征图进行attention调制
            for i, xi in enumerate(x):
                original_features.append(xi.clone())  # 保存原始特征
                modulated_xi = self.attention_heads[i](xi, coords, targets)
                modulated_features.append(modulated_xi)
                x[i] = modulated_xi  # 使用调制后的特征

            # 使用调制后的特征进行标准检测
            if self.end2end:
                detection_outputs = self.forward_end2end(x)
            else:
                for i in range(self.nl):
                    x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
                detection_outputs = x
    ...
    def compute_attention_loss(self, original_features, modulated_features, targets):
        """
        计算attention调制损失
        """
        return self.attention_loss_fn(original_features, modulated_features, targets)